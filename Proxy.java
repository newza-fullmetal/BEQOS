...
//                CSeqHeader cseqtete = (CSeqHeader)request.getHeader(CSeqHeader.NAME);
//                
//                 if ((cseqtete.getMethod() == "INVITE" || cseqtete.getMethod() == "BYE") && false){
//                	 if (forwardRequest2BB(request, ch,cseqtete.getMethod())){
//                		 forwardRequest(targetURI, request, sipProvider, serverTransaction, serverTransaction != null & !requestMustBeProcessedStatelessly); 
//                	 }
//                	 else{
//                		 System.out.println("CALL NOT ACCEPTED BY BB");
//                         Response cancelResponse = messageFactory.createResponse(Response.DECLINE, request);
...
//                 }
    ...
    public void forwardRequest(URI targetURI, Request request, SipProvider sipProvider, ServerTransaction serverTransaction, boolean statefullForwarding) throws InvalidArgumentException, ParseException, SipException
    {
    	...
    	CSeqHeader cseqtete = (CSeqHeader)request.getHeader(CSeqHeader.NAME);
        String method = cseqtete.getMethod();
       
        /**
         * CHECK if we are in the request situation (INVITE or BYE) where it is the only moment we have to interrupt. 
         * After we check if BB is ok or not 
         * If we are not in the BEQOS sitution, we just execute the original code
         */
       	 //if (((method == "INVITE" || method == "BYE" ) && forwardRequest2BB(request, targetURI ,method)) || !BEQOS ){
         if ((forwardRequest2BB(request, targetURI ,method)) || !BEQOS ){
    ...
    /**
	 * Get a sipadress and return either the port or the adress with a parser technic 
	 * @param aLine : String; SIP ADRESS
	 * @param type : integer; 0 for adress, 1 for port
	 * @return String; The value of the Adress or the port
	 */
	 static String SIPAdress(String aLine, int type){
	        ...
	        if (scanner.hasNext()){
	          ...
	          // Use a new scanner of the other part of the line
	          Scanner scanner2 = new Scanner(value);
	          scanner2.useDelimiter(":");
	          if (scanner2.hasNext()){
	        	  adress = scanner2.next();
	        	  String value2 = scanner2.next();
	        	  // Use a last scanner of the last part of the line
	        	  Scanner scanner3 = new Scanner(value2);
	        	  scanner3.useDelimiter(";");
	        	  if (scanner3.hasNext()){
	        		  port = scanner3.next();
	        		  if (port.contains(">")){
	        			  // Used for real phone
	        			  Scanner scanner4 = new Scanner(port);
	 ...
	 /**
	  * This function is called each time we receive an Invite or a bye, it asks to the BB if the ressources are available. 
	  ...
	  */
	    private boolean forwardRequest2BB(Request request, URI targetURI, String method){
	    	System.out.println();
	    	System.out.println();
	    	System.out.println();
	    	System.out.println("Appel de la fonction forwarRequest2BB");
	    	...
	    	SipURI target = (SipURI) targetURI;
	    			 ...
	    			 */
	    			// Integer to signal if it is a reservation or not
	    			int requesttype = -1;
	    			...
	    			if (contactHeader != null ){
		    			...
		    			System.out.println(contactHeader.getAddress());
		    			 srcadress=SIPAdress(contactHeader.getAddress().toString(), 0);
		    			System.out.println("address of caller :" + srcadress );
		    			 srcport= SIPAdress(contactHeader.getAddress().toString(), 1);
		    			System.out.println("port of caller :" + srcport);
	    			}else{
	    				System.out.println(request.getRequestURI());
		    		...
		    		if (targetURI != null){
		    			...
		    			System.out.println("address of destination :" + destadress );
		    			 destport=  target.getPort();
		    			System.out.println("port of destination :" + destport);
	    			}
	    			
	    			if (method == "INVITE"){
	    				requesttype=1; // reservation of ressources
	    			}
	    			else if (method == "BYE" || method == "CANCEL"){
	    				requesttype=0; //Liberation of ressources
	    			}
	    			else{
	    				System.out.println("request type :" +requesttype);
	    				System.out.println("Just forward");
	    				return true;
	    			}
	    			...
	    			if (requesttype != -1){
		    			...
		    			InetAddress IP = null;
		    			try {
		    				// Adress given by the BB group
		    				IP = InetAddress.getByName("192.168.1.8");
		    			} catch (UnknownHostException e1) {
		    				e1.printStackTrace();
		    			}
		    			int Port = 50644; 
						String datatosend = requesttype+";"+srcadress+";"+srcport+";"+destadress+";"+destport+";"+ debit + "\n"; // INFORMATION TO SEND
						...
						try {
							 ...
							 * SEND THE DATA
							 */
							System.out.println("message sent to BB:"+datatosend);
							 ...
							 * WAINTING FOR ANSWER
							 */
							datatorcv = in.readLine();
							System.out.println("Answer from BB :" + datatorcv);
							if (datatorcv.contains("NOT OK")){ // RESERVATION REFUSED
	
								System.out.println("RESERVATION NOT OK");
								reserveok = false;
							}else if (datatorcv.contains("OK")){ // RESERVATION ACCEPTED
								System.out.println("RESERVATION / LIBERATION OK");
	    				...
	    				// SEND TRUE if not INVITE, CANCEL OR BYE
	    				reserveok = true;
	    			}
